...
namespace RaknetCS.Protocol.Raknet
{
    public class FrameSetPacket
    {
        ...
        public int Size()
        {
            ...
            Reliability r = GetReliability();

            if (r == Reliability.Reliable || r == Reliability.ReliableOrdered || r == Reliability.ReliableSequenced)
            {
                ret += 3;
            }
            if (r == Reliability.UnreliableSequenced || r == Reliability.ReliableSequenced)
            {
                ret += 3;
            }
            if (r == Reliability.ReliableOrdered || r == Reliability.ReliableSequenced)
            {
                ret += 4;
            }
            if ((flags & 16) != 0)
