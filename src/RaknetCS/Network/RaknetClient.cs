...
namespace RaknetCS.Network
{
    public class RaknetClient
    {
        ...
        {
            OpenConnectionReply1 reply1Packet = new OpenConnectionReply1(data);
            reply1Packet.Deserialize();
            OpenConnectionRequest2 request2packet = new OpenConnectionRequest2(true, reply1Packet.use_encryption != 0, reply1Packet.cookie, false, peer_addr, reply1Packet.mtu_size, guid);
            byte[] request2Buf = request2packet.Serialize();
            Socket.Send(peer_addr, request2Buf);
        }

        private void HandleOpenConnectionReply2(IPEndPoint peer_addr, byte[] data)
        {
            OpenConnectionReply2 reply2Packet = new OpenConnectionReply2(data);
            reply2Packet.Deserialize();
        ...
        {
            OpenConnectionRequest1 request1Packet = new OpenConnectionRequest1(true, rak_version, Common.RAKNET_CLIENT_MTU);
            byte[] request1Buf = request1Packet.Serialize();
